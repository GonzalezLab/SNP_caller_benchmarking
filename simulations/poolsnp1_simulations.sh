...
POOLSNP_PATH=$DIRDATA/PoolSNP #path to PoolSNP caller
...
#run PoolSNP with miss-fraction = 0.1
for (( REP=1; REP<=$NUMREPS; REP++ ))
do
    $POOLSNP_PATH/PoolSNP.sh \
	mpileup=$MPILEUP/$NUMINV.$PLOIDY.$AVCOV/$RUNPREFIX.POOLS.replicate$REP.haploidcov$AVCOVREF.mpileup \
	output=$POOLSNP/$NUMINV.$PLOIDY.$AVCOV/$RUNPREFIX.POOLS.replicate$REP.haploidcov$AVCOVREF.output \
