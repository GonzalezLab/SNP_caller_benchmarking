...
#Run SNAPE with informative prior
...
do   
	...
	if [ ! -d $DIR/PILEUP_MOD ]
	then
	    snape-pooled -nchr $PLOIDY -theta 0.01 -D 0.10 -fold unfolded  -priortype informative < \
		$DIR/$RUNPREFIX.POOL$POOL.replicate$REP.haploidcov$AVCOVREF.pileup \
		> $SNAPE/$NUMINV.$PLOIDY.$AVCOV/$RUNPREFIX.POOL$POOL.replicate$REP.haploidcov$AVCOVREF.snape_inf.out
	else
	    snape-pooled -nchr $PLOIDY -theta 0.01 -D 0.10 -fold unfolded  -priortype informative < \
...
do
	for (( POOL=1; POOL<=$POOLNUM; POOL++ ))
	do		   
		cat <<- 'EOF' > $SNAPE/$NUMINV.$PLOIDY.$AVCOV/$RUNPREFIX.POOL$POOL.replicate$REP.haploidcov$AVCOVREF.snape_inf.out.R
		x <- read.table(file="'$SNAPE/$NUMINV.$PLOIDY.$AVCOV/$RUNPREFIX.POOL$POOL.replicate$REP.haploidcov$AVCOVREF.snape_inf.out'", na.strings="NA", fill = TRUE, header=F)
		x.filtered  <- x[as.numeric(x[, 9]) >= 0.90,  ]
		write.table(x.filtered, file="'$SNAPE/$NUMINV.$PLOIDY.$AVCOV/$RUNPREFIX.POOL$POOL.replicate$REP.haploidcov$AVCOVREF.snape_inf.filtered.out'", sep="\t", quote=F, col.names=F, row.names=F)
		EOF
		R --vanilla < $SNAPE/$NUMINV.$PLOIDY.$AVCOV/$RUNPREFIX.POOL$POOL.replicate$REP.haploidcov$AVCOVREF.snape_inf.out.R   
	done
done
rm $SNAPE/$NUMINV.$PLOIDY.$AVCOV/*snape_inf.out.R